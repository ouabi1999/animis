...
product_orders = db.Table('product_orders',
    db.Column('order_id', db.String, db.ForeignKey('orders.id'), primary_key=True),
    db.Column('product_id', db.String, db.ForeignKey('products.id'), primary_key=True)
)
...
class Products(db.Model):
    ...
    orders = db.relationship('Orders', secondary = product_orders, backref='products', lazy=True)
    globalCoupon = db.relationship("GlobalCoupon", backref="products", lazy = True, cascade="all, delete-orphan" )
    

    def __str__(self):
        return f'{self.id} {self.seo} {self.created_at} {self.series} {self.title} {self.globalCoupon} {self.orders}  {self.shipping_Method} {self.pics_info} {self.product_type} {self.ratings} {self.colors} {self.tags} {self.availability} {self.category} {self.discount} {self.price} {self.sizes}{self.reviews}{self.quantity}{self.description}'
def productInfo_serializer(info):
    rats = [*map(ratings_serializer , (info.ratings))]
    orders = [*map(orders_serializer , (info.orders))]
...
class Orders(db.Model):
    ...
    ordered_products = db.Column(db.PickleType())
    ...
    def __str__(self):      
        return f'{self.id} {self.orderd_at} {self.paymentMethod}{self.deliveryTime} {self.trackingNumber} {self.deliveryStatus}{self.ordered_products} {self.user_id} {self.firstName} {self.lastName} {self.shippingMethod} {self.shippingPrice} {self.totalPrice} {self.email} {self.address1}{self.address2} {self.country}{self.city} {self.state} {self.zipcode}'
